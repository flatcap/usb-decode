...
static int test_device (uint16_t vid, uint16_t pid)
{
	...
	for (i=0; i<nb_ifaces; i++) {
		...
		for (j=0; j<conf_desc->usb_interface[i].num_altsetting; j++) {
			...
			printf ("   Class.SubClass.Protocol: %02X.%02X.%02X\n", conf_desc->usb_interface[i].altsetting[j].bInterfaceClass, conf_desc->usb_interface[i].altsetting[j].bInterfaceSubClass, conf_desc->usb_interface[i].altsetting[j].bInterfaceProtocol);
